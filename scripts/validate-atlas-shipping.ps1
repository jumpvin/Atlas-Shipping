...
if (-not (Test-Path $entry)) { $failures.Add('plugin-entry-missing') }
  ...
  if ($text -notmatch '(?m)^ \* Version: 0\.1\.7\s*$') { $failures.Add('plugin-header-version-mismatch') }
  if ($text -notmatch "ATLAS_SHIPPING_VERSION', '0\.1\.7'") { $failures.Add('plugin-version-constant-mismatch') }
...
$zipPath = Join-Path $RepositoryRoot 'build/dev/atlas-shipping-0.1.7.zip'
...
$result = [ordered]@{schema_version=1;result=$(if($failures.Count){'failed'}else{'passed'});product_build='0.1.7';schema_version_product='0.1.4';artifact='build/dev/atlas-shipping-0.1.7.zip';failures=@($failures)}
