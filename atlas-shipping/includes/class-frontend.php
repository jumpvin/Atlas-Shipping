...
class Frontend {
 ...
 public static function routes(){ return array('home','my-requests','all-requests','request-detail','new-request','needs-attention','coordinator-handoff','settings','profile'); }
 ...
 public function render_shortcode(){
  ...
  if($identity){
   ...
   foreach(array('Coordinator Handoff','Coordinator authorization is required.','Submitted requests ready for shipper handoff.','No submitted requests need attention.','Review handoff','Back to Needs Attention','Operational request','Items','PM transportation preference','Final outbound decision','Final vehicle / equipment','Choose…','Other equipment','Coordinator notes','Sent at','Shipper loads at pickup','Shipper unloads at delivery','Outbound preview and confirmation','This action creates an immutable shipper-handoff snapshot and locks PM editing.','Send to Shipper','Request sent to shipper. Immutable snapshot created.','Unable to load coordinator workflow.')as$text){$editor_strings[$text]=__($text,'atlas-shipping');}
   $app_config=array(
